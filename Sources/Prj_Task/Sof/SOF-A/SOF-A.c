...
#define CUR_GAIN 10000;
    ...
    // calculate max cur from curve  线性查表
...
void SOF_ModuleCbk(void)
{
    ...
    if(SOF_TmCnt.cnt >= TM_MS(10))
    {
        ...
        wtmp = Bat_GetSumV(ACC_SUMV);   //0.1v/bit
        lwtmp1 = wtmp;
        lwtmp1*= SOF_CalcValue.PowLmt.SOF_PulseCur.Discharge; 
        lwtmp1 /=10000;  //0.1kw/bit
        SOF_CalcValue.PowLmt.SOF_PulsePow.Discharge = (uWord)lwtmp1;  
        
        lwtmp2 = wtmp;
        lwtmp2*= SOF_CalcValue.PowLmt.SOF_PulseCur.Feedback;
        lwtmp2/=10000; //0.1kw/bit 
        SOF_CalcValue.PowLmt.SOF_PulsePow.Feedback = (uWord)lwtmp2;
