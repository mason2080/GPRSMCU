...
{
    /* deal GB protocol  */
    if(GbChgM_TmCnt.CbkCnt >= TM_MS(10))
    {
       GbChgM_TmCnt.CbkCnt = 0;
       GbChgM_LinkModeJudge();  
       
       //Chg_UpdateDcLinkSt(1);  //for test 
...
void GbChgM_LinkModeJudge(void)
{
    ...
    switch(lby_gb_chg_mode)
    {
      ...
      case AC_CHG_MODE3_C:
      {
          if((lwd_cc_volt > CC_V_L_16AMODE) && (lwd_cc_volt < CC_V_H_16AMODE))
          {
              lby_ac_link_st = GB_FLAG_ENABLE;
              lwd_cc_cur = 160;
          }
          else if((lwd_cc_volt > CC_V_L_32AMODE) && (lwd_cc_volt < CC_V_H_32AMODE)) 
          {
              lby_ac_link_st = GB_FLAG_ENABLE;
              lwd_cc_cur = 320;
          }
...
void GbChgM_FlowProc(void)
{
    ...
    case ChgStep5:
    {
        ...
        if( (GB_FLAG_DISABLE == lby_chg_enable_ext)		 /* BMS charge end */
                ...
                || (GB_FLAG_DISABLE == lby_dc_link_st) ) /* charge link open */
        {
            lwd_rx_flg &= ~CST_RX;
