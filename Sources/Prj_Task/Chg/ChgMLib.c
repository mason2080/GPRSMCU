...
uByte ChgM_CellvHighJudge(void)        /* Cell voltage more than 3.8V */
{
    ...
    {
        if((ChgM_Ctrl.Data.MaxV >= CHGM_ERR_MAX_CELLV) 
         || (ChgM_Ctrl.Data.MaxV >= Err_GetAlarmThrhdVal(ERR_BAT_CELLV_HIGH,1)))
        {
            CellVHigh = 1;
        }
    }
...
uByte ChgM_SumvHighJudge(void)     /* Sum voltage more than 390V */
{
    ...
    if((ChgM_Ctrl.Data.SumV >= CHGM_ERR_MAX_SUMV)
      || (ChgM_Ctrl.Data.SumV >= Err_GetAlarmThrhdVal(ERR_BAT_SUMV_HIGH,1)))
    {
        SumVHigh = 1;
    }
...
uByte ChgM_TemperHighJudge(void)
{
    ...
    if((CHGM_FLAG_ENABLE == ChgM_Ctrl.Flag.BmsReady) && (ChgM_Ctrl.Data.MaxT < 0xF0))
    {
        ...
        if(ChgM_Ctrl.Data.MaxT >= Chg_GetPermitMaxT())
        {
            TemperHigh = 1;
        }
    }
...
uByte ChgM_TemperLowJudge(void)
{
    ...
    if((CHGM_FLAG_ENABLE == ChgM_Ctrl.Flag.BmsReady) && (ChgM_Ctrl.Data.MinT < 0xF0))
    {
        ...
        if(ChgM_Ctrl.Data.MinT  <= Chg_GetPermitMinT())
        {
            TemperLow = 1;
        }
    }
...
uByte ChgM_CellVLowJudge(void)
{
    ...
    if((CHGM_FLAG_ENABLE == ChgM_Ctrl.Flag.BmsReady) && (ChgM_Ctrl.Data.MinV < 0xFFF0))
    {
        ...
        if(ChgM_Ctrl.Data.MinV < Chg_GetPermitMinCellV())
        {
            CellVLow = 1;
        }
    }
...
{
    uWord wtmp;
#ifdef CHG_CTL_STOP_CHG_CUR
    wtmp = ChgM_Ctrl.Data.MinChgCur;
#else
    wtmp = ChgM_Ctrl.Data.RatedCap / 10; // 0.1C
#endif
    if( (ChgM_Ctrl.Data.PmtChgCur <= wtmp)
            || (ChgM_Ctrl.Data.PmtChgCur <= ChgM_Ctrl.Data.MinChgCur) )
    {
