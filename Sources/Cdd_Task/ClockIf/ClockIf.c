...
GeneralReturnType ClockIf_ReadTime(sClockIf_TimeType *time)	 
{
    ...
    uByte btmp;
    GeneralReturnType rslt;
    byte year,month,day,minute,second,hour;
    ...
    while(i < 5)
    {
        ...
        // BCD to DEC
        second = (gby_RtcTime[5] >> 4) * 10 + (gby_RtcTime[5] & 0x0f); 
        minute = (gby_RtcTime[4] >> 4) * 10 + (gby_RtcTime[4] & 0x0f);
        hour = (gby_RtcTime[3] >> 4) * 10 + (gby_RtcTime[3] & 0x0f);
        day = (gby_RtcTime[2] >> 4) * 10 + (gby_RtcTime[2] & 0x0f);
        month = (gby_RtcTime[1] >> 4) * 10 + (gby_RtcTime[1] & 0x0f);
        year = (gby_RtcTime[0] >> 4) * 10 + (gby_RtcTime[0] & 0x0f);
