...
static void SumvIf_InitIO(eSumv_IdNoType Sumv_IdNo)
{
    ...
    //(*pDevice->Cpu_Sumv_Rdy.SetVal)();
    (*pDevice->Sumv_Pow.SetVal)();

    (*pDevice->Cpu_Sumv_Sda.SetIoDirOut)();
    //(*pDevice->Cpu_Sumv_Scl.SetIoDirOut)();
    //(*pDevice->Cpu_Sumv_Rdy.SetIoDirIn)();
    //(*pDevice->Sumv_Pow.SetIoDirOut)();
}
...
GeneralReturnType SumvIf_StartConvert(eSumv_IdNoType Sumv_IdNo)
{
   ...
   if((Sumv_IdNo >= 0) && (Sumv_IdNo <= 2)) 
   {
       for (i = 0; i < 3; i++)
       {
           if( (SUMV_INNER_ID == Sumv_IdNo) || (SUMV_EXTER_ID == Sumv_IdNo))
           {
               config = SUMV_CONTINUE_CONV_AIN1_128SPS;
           ...
           {
               config = SUMVBAK_CONTINUE_CONV_AIN2_128SPS;
