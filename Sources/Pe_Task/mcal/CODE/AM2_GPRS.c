...
static AM2_GPRS_TComData BufferRead;   /* Input char for SCI commmunication */
...
byte AM2_GPRS_SendChar(AM2_GPRS_TComData Chr)
{
  ...
  EnterCritical();                     /* Enter the critical section */
  (void) SCI2SR1;                      /* Reset interrupt request flag */
  SCI2DRL = (byte)Chr;                 /* Store char to transmitter register */
  SCI2CR2_TIE = 1U;                    /* Enable transmit interrupt */
...
/*
** ===================================================================
**     Method      :  AM2_GPRS_InterruptRx (component AsynchroSerial)
**
**     Description :
...
static void AM2_GPRS_InterruptRx(void)
{
  AM2_GPRS_TComData Data;              /* Temporary variable for data */
  byte OnFlags = 0U;                   /* Temporary variable for flags */

  Data = SCI2DRL;                      /* Read data from the receiver */
  if(SerFlag & CHAR_IN_RX) {           /* Is a character already present in the receive buffer? */
    ...
    if(OnFlags & ON_RX_CHAR) {         /* Is OnRxChar flag set? */
      AM2_GPRS_OnRxChar();             /* If yes then invoke user event */
    }
}

/*
** ===================================================================
**     Method      :  AM2_GPRS_InterruptTx (component AsynchroSerial)
**
**     Description :
**         The method services the receive interrupt of the selected 
**         peripheral(s) and eventually invokes the component's event(s).
...
#pragma INLINE                         /* Directs the Compiler to inline the next function in the source */
static void AM2_GPRS_InterruptTx(void)
{
  byte OnFlags = 0U;                   /* Temporary variable for flags */
  ...
  SCI2CR2_TIE = 0U;                    /* Disable transmit interrupt */
  if(OnFlags & ON_TX_CHAR) {           /* Is flag "OnTxChar" set? */
    AM2_GPRS_OnTxChar();               /* If yes then invoke user event */
  }
}
...
ISR(AM2_GPRS_Interrupt)
{
  byte StatReg = getReg(SCI2SR1);
  if (StatReg & (SCI2SR1_OR_MASK | SCI2SR1_FE_MASK | SCI2SR1_NF_MASK)) { /* Is any error flag set? */
    SerFlag |= COMMON_ERR;             /* If yes then set an internal flag */
    (void) SCI2DRL;                    /* Dummy read of data register - clear error bits */
    StatReg &= (byte)(~(byte)SCI2SR1_RDRF_MASK); /* Clear the receive data flag to discard the errorneous data */
  }
  if (StatReg & SCI2SR1_RDRF_MASK) {   /* Is the receiver interrupt flag set? */
    AM2_GPRS_InterruptRx();            /* If yes, then invoke the internal service routine. This routine is inlined. */
  }
  if (SCI2CR2_TIE) {                   /* Is the transmitter interrupt enabled? */
    if (StatReg & SCI2SR1_TDRE_MASK) { /* Is the transmitter interrupt flag set? */
      AM2_GPRS_InterruptTx();          /* If yes, then invoke the internal service routine. This routine is inlined. */
    }
  }
}
...
void AM2_GPRS_Init(void)
{
  ...
  setReg8(SCI2CR1, 0x00U);              
  /* SCI2SR2: AMAP=1,??=0,??=0,TXPOL=0,RXPOL=0,BRK13=0,TXDIR=0,RAF=0 */
  setReg8(SCI2SR2, 0x80U);             /* Switch to the alternative register set */ 
  /* SCI2ASR1: RXEDGIF=1,??=0,??=0,??=0,??=0,BERRV=0,BERRIF=1,BKDIF=1 */
  setReg8(SCI2ASR1, 0x83U);            /* Clear alternative status flags */ 
  /* SCI2ACR1: RXEDGIE=0,??=0,??=0,??=0,??=0,??=0,BERRIE=0,BKDIE=0 */
  setReg8(SCI2ACR1, 0x00U);             
  /* SCI2ACR2: ??=0,??=0,??=0,??=0,??=0,BERRM1=0,BERRM0=0,BKDFE=0 */
  setReg8(SCI2ACR2, 0x00U);             
  /* SCI2SR2: AMAP=0,??=0,??=0,TXPOL=0,RXPOL=0,BRK13=0,TXDIR=0,RAF=0 */
  setReg8(SCI2SR2, 0x00U);             /* Switch to the normal register set */ 
  (void) SCI2SR1;                      /* Reset interrupt request flags */
  /* SCI2CR2: TIE=0,TCIE=0,RIE=0,ILIE=0,TE=0,RE=0,RWU=0,SBK=0 */
  SCI2CR2 = 0U;                        /* Disable error interrupts */
  SCI2BD = 0x0138U;                    /* Set prescaler bits */
  SCI2CR2 |= (SCI2CR2_TE_MASK | SCI2CR2_RE_MASK | SCI2CR2_RIE_MASK); /* Enable transmitter, Enable receiver, Enable receiver interrupt */
}
