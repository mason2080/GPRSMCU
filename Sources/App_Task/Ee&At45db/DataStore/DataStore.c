...
static void DataStore_WriteRecord(void);
...
{    
   uWord rslt = 0;
   if(DataStore_TmCnt.SavePageCnt >= TM_MS(361000))
   {
...
#endif        
    if(DataStore_TmCnt.cnt >= TM_MS(30))
    {
...
// Trans para: void
//************************************************************/
static void DataStore_WriteRecord()
{
    uByte i;
    ...
    for(i = 0; i < 5; i++)
    {
        ...
        if((status & FLASH_READY) == FLASH_READY)
        {
...
void DataStore_ErasePage(uWord page)
{
    (*NvmIf_FlashOperation.PageErase)(page);
}
...
GeneralReturnType DataStore_ReadAndTxData(sCanIf_MsgType msg)
{
    ...
    for(;;)
    {
        ...
        /* judge whether power off */
       /* if(DATASTORE_AWAKEST_OFF == BmsSys_GetAwakeSt())
        {
            count++;
        }
