...
void EE_Init(void)
{
    ...
    if((uWord)EE_ERR_INITEE == rslt[0])
    {
        //rslt[0] = Err_UpdateHwErrLevel(ERR_HW_EE_EXT, ERR_LEVEL_TWO);
        //rslt[0] = Err_UpdateHwErrDetail(ERR_HW_EE_EXT, EE_HDERR_INIT);
    }            
...
void EE_MainCbk(void)
{
    ...
    if(EE_TmCnt.cnt >= TM_10MS)
    {
        ...
        /*if(LVM_STATE_OFF == BmsSys_GetAwakeSt())
        {
            if(gby_PowerDownCnt < POWER_DOWN_CNT_1000MS)
            {
                gby_PowerDownCnt++;
            }
        }
