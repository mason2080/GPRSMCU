...
void Bat_CalcAllMaxMin(void)
{
   ...
   // uWord total_serial_num, total_temper_num;
    ...
    for(bmu_no = start_bmu_no ; bmu_no < total_bmu_num; bmu_no++)
    {
    ...
    wtmp = 0;  
    for(i = start_bmu_no; i < max_v_bmu_no ; i++)
    {
    ...
    wtmp = 0;
    for(i = start_bmu_no; i < min_v_bmu_no; i++)
    {
        wtmp +=  Bat_GetPackSerialNum(i);
    }
    ...
    wtmp = 0;
    for(i = start_bmu_no; i < max_t_bmu_no; i++)
    {
    ...
    wtmp = 0;
    for(i = start_bmu_no; i < min_t_bmu_no; i++)
    {
        wtmp +=  Bat_GetPackTemperNum(i);
    }
 ...
 #ifdef BMS_ENABLE_CALC_INTRES   
    wtmp = 0;
    for(i = start_bmu_no; i < max_r_bmu_no ; i++)
    {
        wtmp +=  Bat_GetPackSerialNum(i);
    }
    ...
    wtmp = 0;   
    for(i = start_bmu_no; i < min_r_bmu_no; i++)
    {
        wtmp +=  Bat_GetPackSerialNum(i);
    }
